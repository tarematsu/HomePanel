...
describe("telemetry retention", () => {
  it("removes old applied receipts and buckets while retaining recoverable pending samples", async () => {
    const now = Date.now();
    ...
    const insertSample = (sequence: number, observedAt: number, applied: number) => env.DB.prepare(
      `INSERT INTO environment_samples(
       ...
       ) VALUES('ci-device',?1,?2,700,NULL,NULL,NULL,NULL,?3)`,
    ...
    const insertBucket = (bucketAt: number) => env.DB.prepare(
      `INSERT INTO environment_buckets(
       ...
       ) VALUES('ci-device',?1,1,700,1,0,0,0,0)`,
    ).bind(bucketAt);

    await env.DB.batch([
      insertSample(1, now - 32 * day, 1),
      insertSample(2, now - 32 * day, 0),
      insertSample(3, now - 91 * day, 0),
    ...
    expect(buckets.results).toEqual([{ bucket_at: now - 6 * day }]);
  });
});
