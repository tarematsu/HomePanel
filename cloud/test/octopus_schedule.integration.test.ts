...
describe("Octopus schedule", () => {
  it("normalizes the Octopus job to an hourly interval", async () => {
    vi.useFakeTimers();
    ...
    await env.DB.prepare(
      `INSERT INTO jobs(name,interval_seconds,next_run_at,lease_until,last_success_at,last_error,consecutive_failures)
       VALUES('octopus',21600,?1,NULL,NULL,NULL,0)
       ON CONFLICT(name) DO UPDATE SET interval_seconds=21600,next_run_at=?1`,
    ).bind(nowSeconds + 21_600).run();
    ...
    expect(job?.interval_seconds).toBe(3600);
    expect(job?.next_run_at).toBe(nowSeconds + 3600);
  });

  it("preserves an already queued immediate refresh", async () => {
    await env.DB.prepare(
      "UPDATE jobs SET interval_seconds=21600,next_run_at=0 WHERE name='octopus'",
    ).run();
    ...
    expect(job).toEqual({ interval_seconds: 3600, next_run_at: 0 });
  });
});
