...
<div class="row" style="margin-top:12px"><button id="load" class="primary">クラウド設定を読み込む</button><button id="save" class="primary">保存して端末を再起動</button></div><div id="status" class="status"></div><div class="muted">Cloudflare同期は900秒、CO2・温湿度アップロードは60分で固定されます。Spotifyは端末から直接取得します。</div></section>
...
const merge=(base,extra)=>{const output=structuredClone(base);if(!extra||typeof extra!=="object"||Array.isArray(extra))return output;for(const [key,value] of Object.entries(extra)){if(value&&typeof value==="object"&&!Array.isArray(value)&&output[key]&&typeof output[key]==="object")output[key]=merge(output[key],value);else output[key]=value}output.cloudPollSeconds=900;output.telemetryMinutes=60;return output};
